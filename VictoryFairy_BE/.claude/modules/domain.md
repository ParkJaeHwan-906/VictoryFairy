...
## 주의 / 열려있는 것
- `UserRepository`(existsByEmail/existsByTel), `UserAccountRepository`(findByUser_EmailAndExitAtIsNull/existsByNickname/**findActiveIdByUid**), `UserRefreshTokenRepository`(findByRefreshToken/deleteByUserAccount/expireValidTokens), `TeamRepository`(**findAllByOrderByNameAsc** — `user` 모듈 `TeamService`의 구단 목록 조회용, 정렬은 DB 단독 수행), `PlayerRepository`(**findAllByOrderByNameAsc**/**findAllByTeam_IdOrderByNameAsc** — 구단별 선수 목록 조회용, `Team_Id`로 끊어 `team.id`로만 해석되게 함), `StadiumRepository`(findByName), `GameStatusRepository`(findByName), `ChatroomRepository`(아래), `ChatRepository`(아래), `UserSupportTeamRepository`(아래), `UserSupportPlayerRepository`(아래) 외에는 **전부 `JpaRepository` 뼈대뿐** — `BatResultRepository`/`BatterRecordRepository`/`PitcherRecordRepository`/`PitchResultRepository`/`GameRepository`는 커스텀 조회 메서드 없음. `findByName` 둘은 시드/크롤러의 lookup-or-create 용. `UserRefreshTokenRepository.deleteByUserAccount`는 **소비처가 0개인 죽은 메서드**(`logout`은 `findByRefreshToken`→`delete`를 씀)
...
- `player`/`stadium`/`game`/`record` 엔티티를 소비하는 서비스·컨트롤러는 아직 없음. `team`은 `user` 모듈의 `TeamController`/`TeamService`(구단 목록 조회, 쓰기 경로 없음)가 실사용한다. `chat` 계열(`Chatroom`·`Chat`)은 `quiz` 모듈의 `ChatService`/`ChatController`가 실사용한다. `user` 엔티티는 `user` 모듈과 `web-support`(`JwtAuthenticationFilter`가 `UserAccountRepository` 사용)가 쓴다. `support` 계열(`UserSupportTeam`·`UserSupportPlayer`)은 **아직 소비하는 서비스·컨트롤러가 없다**(domain 리포지토리만 존재)
- **응원 취소/재응원 설계**: `UserSupportTeam`/`UserSupportPlayer`는 취소·재응원을 이력 행으로 쌓지 않고 같은 행의 `oppose`(DATETIME, null=응원 중)를 토글한다 — `oppose(LocalDateTime)`(이미 취소 상태면 no-op으로 최초 취소 시각 보존)/`support()`(재활성)/`isOpposed()`. UNIQUE(`user_account_id`, `team_id`|`player_id`)에 `oppose`를 넣지 않은 이유는 MySQL에 partial unique index가 없어서가 아니라, 재응원이 새 행이 아닌 같은 행 재활성이라 (계정, 대상) 조합 자체가 하나뿐이기 때문이다(`oppose`를 넣으면 오히려 같은 조합의 중복 행을 허용하게 됨). **"한 사용자는 구단을 1개만 응원한다"는 스키마 제약이 아니라 서비스 정책**이다 — 쓰기 경로(아직 미구현)가 지켜야 하며, `UserSupportTeamRepository.findByUserAccount_IdAndOpposeIsNull`이 `Optional`을 반환하는 것으로만 그 전제가 드러난다(선수는 복수 응원·상한 없음이라 `List`). 이 정책을 강제할 서비스·컨트롤러 소비처가 아직 없다는 점(바로 위 항목)이 이 상태를 더 취약하게 만든다 — 아무도 강제하지 않는 정책이다.
...
- **prod 스키마는 `user` 앱이 만든다.** `UserApplication`이 `@EntityScan("com.skhynix")`로 domain 엔티티를 전부
  ...
  `user_support_team`·`user_support_player`는 아직 이 사이클을 타지 않아 **prod에 미생성 상태**다(2026-07-28 기준) — `user` 앱이 다시 기동해야 반영된다.
