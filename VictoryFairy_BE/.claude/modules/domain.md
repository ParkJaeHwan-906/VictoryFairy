...
- **`game_statuses` 시드 없음**: 아래 5행을 채우는 마이그레이션/시드 도구가 아직 없다. `Game.gameStatus`가 non-null FK라 **시드 전에는 `Game` 저장 자체가 불가능**.
  ...
  py-collector 쪽 `games_sync` 잡(계획 단계, 아직 미착수)이 `positions`와 같은 lookup-or-insert 패턴으로 상태명 행을 자동 생성할 예정이라, 도입되면 이 수동 시드가 불필요해질 수 있다.
