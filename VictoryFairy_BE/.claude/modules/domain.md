...
- `game` — `Game`, `GameStatus`(코드 테이블), `GameLineup`, `Position`(코드 테이블) + `GameRepository`, `GameStatusRepository`, `GameLineupRepository`, `PositionRepository`
...
`User`→`users` · `UserAccount`→`users_account` · `UserRefreshToken`→`users_refreshtoken` · `Team`→`teams` · `Player`→`players` · `Stadium`→`stadiums` · `Game`→`games` · `GameStatus`→`game_statuses` · `GameLineup`→`game_lineups` · `Position`→`positions` · `BatterRecord`→`batter_records` · `PitcherRecord`→`pitcher_records` · `Chatroom`→`chatrooms` · `Chat`→`chats` · `UserSupportTeam`→`user_support_team`(단수, 사용자 확정) · `UserSupportPlayer`→`user_support_player`(단수, 사용자 확정). 새 엔티티에 임의로 단수 테이블명을 쓰지 말 것 — 이 2건은 명시적 예외다.
...
## 주의 / 열려있는 것
- `UserRepository`(existsByEmail/existsByTel), `UserAccountRepository`(findByUser_EmailAndExitAtIsNull/existsByNickname/**findActiveIdByUid**), `UserRefreshTokenRepository`(findByRefreshToken/deleteByUserAccount/expireValidTokens), `TeamRepository`(**findAllByOrderByNameAsc** — `user` 모듈 `TeamService`의 구단 목록 조회용, 정렬은 DB 단독 수행), `PlayerRepository`(**findAllByOrderByNameAsc**/**findAllByTeam_IdOrderByNameAsc**/**findAllByIdInOrderByNameAsc** — 앞 둘은 구단별 선수 목록 조회용(`Team_Id`로 끊어 `team.id`로만 해석되게 함), 마지막은 `user` 모듈 `SupportService`의 응원 선수 응답용 batch 조회(응원 행에서 FK id만 모아 한 번에 가져와 N+1을 막는다)), `StadiumRepository`(findByName), `GameStatusRepository`(findByName), `PositionRepository`(findByName — 셋 다 시드/크롤러의 lookup-or-create 용), `GameLineupRepository`(**findByGameId**/**findByGameIdAndIsStarterTrue** — 뒤엣것이 선발 라인업 필터), `GameRepository`(**findAllByGameDateGreaterThanEqualAndGameDateLessThanOrderByGameDateAsc** — `user` 모듈 `GameService`의 날짜별 경기 목록 조회용, `@EntityGraph(attributePaths = {homeTeam, awayTeam, stadium, gameStatus})`로 N+1 방지. **domain 최초의 `@EntityGraph` 사용처** — 목록은 `GameResponse`가 읽는 연관과 1:1로 유지해야 하며, 응답에 필드를 더하면서 안 고치면 N+1이 부활하고 prod(`open-in-view: false`)에서는 `LazyInitializationException`이 난다), `ChatroomRepository`(아래), `ChatRepository`(아래), `UserSupportTeamRepository`(아래), `UserSupportPlayerRepository`(아래) 외에는 **전부 `JpaRepository` 뼈대뿐** — `BatterRecordRepository`/`PitcherRecordRepository`는 커스텀 조회 메서드 없음. `UserRefreshTokenRepository.deleteByUserAccount`는 **소비처가 0개인 죽은 메서드**(`logout`은 `findByRefreshToken`→`delete`를 씀)
...
- **`game_statuses` 시드 없음**: 아래 5행을 채우는 마이그레이션/시드 도구가 아직 없다. `Game.gameStatus`가 non-null FK라 **시드 전에는 `Game` 저장 자체가 불가능**.
  ...
  py-collector 쪽 `games_sync` 잡은 **구현 완료**(AI 브랜치 `sotaeho/ai/feat-games-status-records` — 머지 대기)이며, `positions`와 같은 lookup-or-insert 패턴으로 상태명 행을 자동 생성하므로 도입되면 이 수동 시드가 불필요해질 수 있다.
