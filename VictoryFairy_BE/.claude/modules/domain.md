...
`User`→`users` · `UserAccount`→`users_account` · `UserRefreshToken`→`users_refreshtoken` · `UserBq`→`users_bq`(사용자 확정 — `UserAccount`/`UserRefreshToken`과 같은 `users_`+한정어 형태라 아래 2건과 달리 규칙 위반이 아니다) · `Team`→`teams` · `Player`→`players` · `Stadium`→`stadiums` · `Game`→`games` · `GameStatus`→`game_statuses` · `BatResult`→`bat_results` · `PitchResult`→`pitch_results` · `BatterRecord`→`batter_records` · `PitcherRecord`→`pitcher_records` · `Chatroom`→`chatrooms` · `Chat`→`chats` · `UserSupportTeam`→`user_support_team`(단수, 사용자 확정) · `UserSupportPlayer`→`user_support_player`(단수, 사용자 확정). 새 엔티티에 임의로 단수 테이블명을 쓰지 말 것 — 이 2건은 명시적 예외다.
...
- 테스트 현황: `domain/src/test` 소스셋(9개 클래스 32개 테스트)에 `GameTest`(3케이스)·`GameLineupTest`(2케이스)·`StadiumTest`(1케이스)·`ChatroomTest`(8케이스, Builder 배선·owner 동일 인스턴스 배선·uid 생성·join/leave/delete 전이)·`ChatTest`(4케이스, Builder 배선·blind/unblind 토글·delete 전이·이미 삭제된 메시지 재삭제 no-op)·`UserSupportTeamTest`/`UserSupportPlayerTest`(각 6케이스, Builder 필드 배선·oppose 초기값 null·oppose()/support() 전이·no-op/멱등·왕복 후 재기록)·`UserAccountPointTest`(1케이스, builder로 만든 신규 계정의 `point` 초기값 0)·`UserBqTest`(1케이스, Builder 필드 배선·`bqScore` 초기값 0)가 있다(Spring 컨텍스트/DB 없이 필드 배선·전이 로직만 확인하는 순수 단위 테스트). `record` 계열은 테스트 없음. `UserAccountWithdrawTest`는 여전히 `domain` 패키지 이름을 그대로 쓴 채 `user/src/test`에 얹혀 있다. 저장소 전체에 H2/Testcontainers/구동 중인 MySQL이 없어 `@DataJpaTest` 라운드트립 검증(FK 제약, nullable 컬럼 매핑, 코드 테이블 FK 저장·복원, `UserSupportTeam`/`UserSupportPlayer`의 UNIQUE 제약·FK CASCADE·타임스탬프·리포지토리 파생 쿼리 포함)은 여전히 보류 상태
