...
- **로컬 자동 시드(dev 전용)**: 기동 시 `infra/sql/teams-init.sql` → `chat-init.sql` → `game-statuses-init.sql` → `quiz-type-init.sql` 을 순서대로 실행해 구단 10개·SYSTEM 계정·구단별 채팅방 10개·경기 상태 5행·퀴즈 유형 2행을 채운다(`application-dev.yaml` 의 `spring.sql.init`, 파일은 `build.gradle` 의 `processResources` 가 `classpath:sql/` 로 복사 — 시드의 단일 출처는 `infra/sql`). 전부 `INSERT ... WHERE NOT EXISTS` 라 매 기동 재실행해도 중복이 없다. `defer-datasource-initialization: true` 로 Hibernate 초기화 뒤에 돈다. **prod 는 해당 없음**(ddl-auto=none + 수동 적용 규칙 유지). dev `ddl-auto=update` 와 맞물려 **빈 DB 에서 `:quiz:bootRun` 한 번이면 스키마 22테이블 + 시드까지 완성**된다(2026-08-07 실측 — 테이블 수는 domain 엔티티가 늘 때마다 바뀌니 이 숫자를 근거로 뭔가를 판정하지 말 것)
