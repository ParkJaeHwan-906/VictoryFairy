...
- `player.controller.PlayerController`(`/players`, GET만) → `player.service.PlayerService`(클래스 레벨 `@Transactional(readOnly = true)`, 쓰기 경로 없음 — 선수 데이터는 py-collector 소유) → `PlayerRepository.findAllByOrderByNameAsc()` / `findAllByTeam_IdOrderByNameAsc(teamId)`. `player.dto.PlayerResponse`(record `id`+`name`, `kboPlayerId`는 py-collector 소유 자연키라 미노출, `team`은 LAZY라 N+1 방지 겸 미노출). **DTO 는 `:domain` 이 아니라 앱 모듈(`user.player.dto`)에 둔다** — API 계약을 공유 모듈에 두면 quiz 까지 끌려간다(초안이 `domain.player.entity` 에 뒀다가 옮긴 자리). 계약은 `docs/requirements/user/player-list.md`(USER-PL-1~12) — 이 문서만 구현 후 사후 작성이다
...
GET /api/member/players                  (?teamId= 선택) → ApiResponse<List<PlayerResponse>> (200, 무인증, name 오름차순, 없는 teamId 는 빈 배열)
...
- **`teamId` 같은 조회 필터에 존재 검증을 붙이지 않는다**: 없는 `teamId`는 404가 아니라 200 + 빈 배열이다. 구단 존재 확인에 조회를 한 번 더 쓰는 대신 이미 공개된 `GET /teams`가 유효 id의 출처라는 전제를 따른다. `teamId`가 숫자가 아니면 컨트롤러 진입 전 타입 변환에서 400이 나는데, 이 경로만 `GlobalExceptionHandler`를 안 타 **`ApiResponse` 래퍼가 아니다**(Spring 기본 `DefaultHandlerExceptionResolver`)
