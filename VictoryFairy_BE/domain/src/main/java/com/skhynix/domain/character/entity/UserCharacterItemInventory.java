 ...
 *
 * <p><b>행의 존재 자체가 "구매했다"이다.</b> 별도 구매 이력 테이블이 없으므로 이 행을 지우면 구매
 * 사실이 사라진다 — 착용 해제는 행 삭제가 아니라 {@code active} 를 끄는 것이다.
 *
 * <p>아래 UNIQUE 가 "하나의 아이템은 영구적으로 한 개만 구매한다"를 보장한다. 서비스의 중복 구매
 * 검사는 409 를 돌려주기 위한 것이고, 이 제약은 그 검사와 INSERT 사이를 파고드는 동시 요청까지 막는다.
 *
 * <p><b>"같은 부위 아이템을 하나만 켠다"는 스키마 제약이 아니라 서비스 정책이다</b> — 부위가 이 테이블이
 * 아니라 {@code character_items.item_type_id} 에 있어 UNIQUE 로 표현할 수 없다.
 */
@Entity
@Table(
...
public class UserCharacterItemInventory {
    ...
    // TINYINT(1) 로 못 박는 이유는 UserCharacterInventory 와 같다.
