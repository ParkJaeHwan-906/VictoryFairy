...
public enum ErrorCode {
    ...
    // 카탈로그에는 있지만 그 계정이 사지 않은 아이템 — 착용 토글의 거절 사유다.
    // ⚠ 위 CHARACTER_ITEM_NOT_FOUND 와 합치지 말 것: 목록 API 가 카탈로그 전체를 having 과 함께
    //   돌려주므로 "없는 아이템"과 "안 산 아이템"은 클라이언트가 이미 구분할 수 있는 상태이고,
    //   한 문구로 뭉치면 FE 가 "구매하기"를 띄울지 "다시 시도"를 띄울지 판단할 근거를 잃는다.
    ...
    // 409 Conflict - 아이템 구매
    // 아이템은 영구적으로 한 개만 살 수 있다(소모품이 아니다). 재구매 시도는 잘못된 요청이 아니라
    // 이미 그 상태에 도달해 있다는 뜻이라 409 다 — QUIZ_ALREADY_SUBMITTED 와 같은 성격.
