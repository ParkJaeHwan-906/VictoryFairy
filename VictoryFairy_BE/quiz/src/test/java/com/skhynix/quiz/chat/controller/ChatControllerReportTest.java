...
/**
 * {@code POST /api/game/chat/rooms/{roomUid}/messages/{messageId}/report}(신고→즉시 blind) 슬라이스 테스트.
 *
...
class ChatControllerReportTest {
    ...
    void reportMessage_normalMessage_returns200() throws Exception {
        ...
        mockMvc.perform(post("/api/game/chat/rooms/{roomUid}/messages/{messageId}/report", ROOM_UID, MESSAGE_ID)
    ...
    void reportMessage_messageNotFound_returns404() throws Exception {
        ...
        mockMvc.perform(post("/api/game/chat/rooms/{roomUid}/messages/{messageId}/report", ROOM_UID, 999L)
                        .with(authenticatedAs(USER_ID)))
