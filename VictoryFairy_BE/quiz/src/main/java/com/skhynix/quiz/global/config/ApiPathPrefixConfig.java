...
/**
 * quiz 모듈의 모든 컨트롤러에 외부 경로 접두사 {@code /api/game}을 전역으로 붙인다.
 *
 * <p>덕분에 컨트롤러는 자기 자원 이름만 갖는다 — {@code @RequestMapping("/chat")}은
 * 실제로 {@code /api/game/chat/**}로 노출된다. 컨트롤러가 늘어나도 접두사를 반복하지 않는다.
 *
 ...
 *       {@code requestMatchers}는 외부 경로를 그대로 쓰면 된다.</li>
 * </ul>
 *
...
public class ApiPathPrefixConfig implements WebMvcConfigurer {
    ...
    public void configurePathMatch(PathMatchConfigurer configurer) {
        configurer.addPathPrefix("/api/game", HandlerTypePredicate.forBasePackage("com.skhynix.quiz"));
    }
}
