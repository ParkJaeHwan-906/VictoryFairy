...
> **최종 갱신** 2026-08-17 — `PATCH /api/users/me/nickname`·`PATCH /api/users/me/password`(내 프로필 수정) 신규 추가. 엔드포인트 2개→4개.
> ⚠ **아직 `main`에 머지되지 않은 브랜치의 내용이다** — 브랜치 `hwannee/be/feat-edit-profile`. 신규 엔드포인트 2개(`PATCH /me/nickname`·`PATCH /me/password`)는 이 브랜치에만 있고, `main`에는 아직 없다.
...
> 최종 변경: 2026-08-17 — 신규 추가(브랜치 `hwannee/be/feat-edit-profile`, 아직 `main` 미병합)
...
**닉네임과 달리 쿨다운이 없다** — 의도된 비대칭이다: 비밀번호가 유출됐을 때 사용자의 자구책은 즉시 교체뿐이라 그 수단을 잠그지 않기 위함이며, 후속 브랜치(`hwannee/be/feat-token-invalidation`)가 다룰 access 토큰 무효화의 트리거를 봉인하지 않기 위함이기도 하다.

⚠ **변경 전에 발급된 access 토큰은 무효화되지 않는다.** 성공 시 새 토큰 쌍을 발급하지만, access 토큰 검증은 서명·만료만 보는 stateless 판정이라 "더 새로운 토큰이 나왔다"를 알 방법이 없다 — 이전 access 토큰은 남은 유효기간(최대 3h) 동안 계속 인증된다. 이 즉시 무효화는 별도 브랜치(`hwannee/be/feat-token-invalidation`, `password_changed_epoch_second` 컬럼+필터 대조 예정, 이 브랜치가 먼저 머지된 뒤 진행)로 분리돼 있다. refresh 토큰만 즉시 만료된다.
...
- (과거 기록, 정정됨) 이전 버전 문서에는 "프로필 **수정**(`PATCH`/`PUT /api/users/me` 류) 엔드포인트는 아직 없다"고 적혀 있었다 — `PATCH /api/users/me/nickname`·`PATCH /api/users/me/password` 추가(2026-08-17, 브랜치 `hwannee/be/feat-edit-profile`)로 더 이상 사실이 아니다.
...
- 요구사항: `docs/requirements/user/withdraw.md`, `docs/requirements/user/me-profile.md`(USER-ME-1~36, 2026-08-06 2차 개정으로 `supportPlayers` 추가·상한 무관 서술 확정), `docs/requirements/user/profile-edit.md`(USER-PE-1~49, 승인됨 2026-08-17 — `PATCH /me/nickname`·`PATCH /me/password`의 출처)
