...
VictoryFairy BE는 Gradle 멀티모듈 프로젝트입니다. 이 문서는 **모듈별 역할**, **인증(JWT) 적용 방향**을 정리합니다.
...
| `quiz` | 애플리케이션 (Spring Boot) | O | 8081 | 퀴즈 도메인. **인증 토큰 검증** |

실행 가능한 애플리케이션은 `user`, `quiz` 두 개이며, 두 앱은 **같은 MySQL DB를 공유**합니다.
