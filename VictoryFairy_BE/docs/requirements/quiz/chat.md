...
3. **`/rt/chat/**`는 자동으로 인증 필수.** quiz `SecurityConfig`가 `/`, `/error`, `GET /health` 외 `anyRequest().authenticated()`이며 미인증은 user의 `RestAuthenticationEntryPoint`로 **401**을 낸다. chat 경로용 SecurityConfig 수정은 불필요.
...
6. **`Chat`에는 외부 식별자(`uid`)가 없고, 추가하지 않는다(Q5 확정).** `chats`는 고write 테이블이라 랜덤 UUID 유니크 인덱스는 삽입 지역성을 해쳐 채택하지 않는다. 메시지를 지목해야 하는 유일한 경로인 신고는 **내부 PK를 room-스코프 경로로 지목**한다: `POST /rt/chat/rooms/{roomUid}/messages/{messageId}/report`. 열거 방어는 인가(방 접근 사용자)·신고 규칙·blind 가역성으로 충당한다. **SSE·히스토리 응답 payload에는 메시지 식별자를 싣지 않는다.**
...
| QUIZ-CHAT-2 | 이벤트 | WHEN 인증된 사용자가 채팅방 목록을 요청하면, THE 시스템 SHALL 소프트삭제되지 않은 방 목록을 200으로 반환한다 | `GET /rt/chat/rooms` → 200, 각 항목에 `roomUid`·`team`·`name` 포함, `deletedAt`이 채워진 방은 목록에 없다. (2026-08-01 개정: 인수 기준에서 `participants`를 제거 — QUIZ-CHAT-52) |
| QUIZ-CHAT-3 | 예외 | IF 존재하지 않거나 소프트삭제된 `roomUid`로 접근하면, THEN THE 시스템 SHALL 404를 반환한다 | 임의 UUID로 `GET /rt/chat/rooms/{roomUid}` → 404. 삭제된 방도 404 |

### B. 인증·인가

| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| QUIZ-CHAT-4 | 예외 | IF `/rt/chat/**` 요청에 유효한 액세스 토큰이 없으면, THEN THE 시스템 SHALL 401을 반환한다 | `Authorization` 헤더 없이 `POST /rt/chat/rooms/{roomUid}/messages` → 401(`RestAuthenticationEntryPoint` 형식). 만료·리프레시 토큰도 401 |
...
| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| QUIZ-CHAT-6 | 이벤트 | WHEN 인증 사용자가 방 구독을 요청하면, THE 시스템 SHALL `text/event-stream` 스트림을 200으로 열어 유지한다 | `GET /rt/chat/rooms/{roomUid}/subscribe`(fetch 기반 폴리필로 `Authorization` 헤더 유지) → 200, `Content-Type: text/event-stream`, 연결이 즉시 끊기지 않는다 |
...
| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| QUIZ-CHAT-10 | 이벤트 | WHEN 인증 사용자가 유효한 content로 메시지 전송을 요청하면, THE 시스템 SHALL `Chat`을 `blind=false`·`deletedAt=null`로 저장하고 저장된 메시지를 201로 반환한다 | `POST /rt/chat/rooms/{roomUid}/messages` `{"content":"안녕"}` → 201, 응답에 `id`·`content`·`senderNickname`(발신자 `UserAccount.nickname`)·`createdAt`, `chats`에 1행 증가 |
...
| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| QUIZ-CHAT-18 | 이벤트 | WHEN 인증 사용자가 방의 과거 메시지를 요청하면, THE 시스템 SHALL 페이지 크기 30·최신순(`createdAt` desc)으로 페이징된 메시지 목록을 200으로 반환한다 | `GET /rt/chat/rooms/{roomUid}/messages?...` → 200, 한 페이지 최대 30건, 최신 메시지가 먼저 |
...
| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| QUIZ-CHAT-20 | 이벤트 | WHEN 인증 사용자가 방 접근 권한 내에서 타인의 메시지를 신고하면, THE 시스템 SHALL 대상 `Chat`을 즉시 `blind=true`로 전환한다(신고 1건으로 즉시 숨김, 임계치·관리자 승인 없음) | `POST /rt/chat/rooms/{roomUid}/messages/{messageId}/report` → 2xx, 대상 메시지 `blind=true` |
...
| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| QUIZ-CHAT-24 | 상태 | WHILE 채팅방이 소프트삭제된 상태이면, THE 시스템 SHALL 목록·조회·구독·전송·히스토리 전부에서 그 방을 404로 취급한다 | 삭제된 방은 `GET /rt/chat/rooms` 목록에 없고, 구독·전송·히스토리 요청은 404. **방을 삭제하는 사용자·엔드포인트 트리거는 이번 범위 밖**(관리자 없음) — "삭제된 상태이면 이렇게 동작한다"는 계약만 정의 |
...
| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| QUIZ-CHAT-52 | 유비쿼터스 | THE 시스템 SHALL 채팅방 목록·상세 응답 본문에서 참여 인원(`participants`) 필드를 제외한다 | `GET /rt/chat/rooms` → 200, 각 항목의 키 집합에 `participants`가 **없다**(값이 `0`/`null`인 것이 아니라 키 자체 부재). `GET /rt/chat/rooms/{roomUid}` → 200, 본문 키 집합에 `participants`가 없다. 두 응답의 방 필드는 `roomUid`·`team`·`name` |
...
8. **문서 정합 — 해소됨(2026-08-01).** `docs/api/chat.md`(당시 `docs/api/quiz.md`)는 필드표에서 `participants`를 빼고 "참여 인원은 노출하지 않음" 절로 교체되었고, `.claude/modules/quiz.md`도 "참여 인원은 어디에도 서빙되지 않는다"·방 응답 `{roomUid, team, name}`으로 정정되었다. 이 개정과 어긋나는 서술은 남아 있지 않다.
