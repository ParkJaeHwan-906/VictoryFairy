# 응원 구단·선수 선택 요구사항
> 상태: **승인됨 · 구현·검증 완료** (2026-07-28) · 모듈: user · 최종 수정: 2026-07-28
...
## 범위
- 포함: 응원 구단 선택/변경 1개, 응원 선수 추가 1개, 응원 선수 취소 1개. 필요한 `ErrorCode` 신설. 응답 DTO
- 제외:
  ...
  - **응원 선수 수 상한** — 도메인 Javadoc이 "복수 허용, 상한 없음"으로 명시
...
| USER-SP-22 | 유비쿼터스 | THE 시스템 SHALL 응원 선수 수에 상한을 두지 않는다 | 한 요청에 다수 선수를 넣어도 개수 때문에 실패하지 않음 |
...
- **신설 `ErrorCode`(`:common`) 3종**: `TEAM_NOT_FOUND`(404) · `PLAYER_NOT_FOUND`(404) · `SUPPORT_TEAM_REQUIRED`(400) · `PLAYER_NOT_IN_SUPPORT_TEAM`(400). 기존 404 는 `CHATROOM_NOT_FOUND`/`CHAT_MESSAGE_NOT_FOUND` 뿐이라 구단·선수용이 없다.
...
| USER-SP-22 | (전용 테스트 없음 — 상한 로직이 코드에 존재하지 않는 것이 근거. 아래 "미커버 영역") |
...
`SupportServiceTest` 21건 · `SupportControllerTest` 19건, 전부 통과.

**미커버 영역(정직하게 기록)**
- **USER-SP-22(상한 없음)**: "제한이 없다"는 부재를 단위 테스트로 증명할 수 없다. 코드에 개수 검사가
  없는 것이 근거이며, 대량 요청의 성능은 별개 문제다(요청당 선수 수만큼 `findByUserAccount_IdAndPlayer_Id`
  조회가 나가므로 수백 건을 한 번에 보내면 느리다 — 화면이 그럴 일이 없다고 보고 최적화하지 않았다).
