...
| USER-PE-32 | 유비쿼터스 | THE 시스템 SHALL 비밀번호 변경 전에 발급된 access 토큰을 무효화하지 않는다 | 변경 직후 변경 전 access 토큰으로 `GET /api/users/me` → 200(401 아님). **새 토큰 쌍(USER-PE-22)을 발급받아도 마찬가지다** — 새 토큰 발급은 이전 access 토큰을 죽이지 않는다. 그 토큰은 남은 유효기간(최대 3h) 동안 계속 인증된다 — **의도된 한계의 명문화이지 바람직한 동작이 아니다**(아래 제약 4, 별도 작업으로 분리됨) |
...
4. **비밀번호 변경에는 탈퇴가 가진 안전망이 없다 — 새 토큰을 발급해도 마찬가지다.** 탈퇴는 `JwtAuthenticationFilter.findActiveIdByUid`가 요청마다 활성 계정을 확인해 access 토큰을 즉시 무력화하지만, 비밀번호를 바꾼 계정은 **여전히 활성**이라 그 조회를 그대로 통과한다. ⚠ **가장 오해하기 쉬운 지점**: USER-PE-22가 새 토큰 쌍을 발급한다고 해서 이전 access 토큰이 무효가 되지 않는다. access 토큰 검증은 **서명과 만료만 보는 stateless 판정**이라 "더 새로운 토큰이 나왔다"는 사실을 알 방법이 없다 — 이전 access 토큰은 최대 3h 그대로 통과한다(USER-PE-32). refresh만 DB 행이 있어 만료시킬 수 있다(USER-PE-30). 이를 없애려면 필터가 매 요청 "비밀번호 변경 시각 이후 발급된 토큰인가"를 대조해야 하는데, 그건 토큰 claim 계약(`sub`=uid만)과 필터 조회를 함께 바꾸는 작업이라 **사용자가 별도 작업으로 분리하기로 확정**했다.
...
- **access 토큰은 비밀번호 변경으로 무효화되지 않는다**(USER-PE-32·제약 4). 새 토큰 쌍을 발급받아도 이전 access 토큰이 최대 3h 살아 있다. **별도 작업으로 분리됨.**
