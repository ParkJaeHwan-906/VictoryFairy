...
- **응원 구단 조회에는 구단명이 필요 없다.** `resolveTeamId()`는 적용 구단의 **id만** 쓰므로 `@EntityGraph`가 붙은 `findWithTeamByUserAccount_IdAndOpposeIsNull`을 쓰면 불필요한 조인이 붙는다(리포지토리 Javadoc이 두 메서드의 용도를 이미 갈라 놓았다).
  > 2026-08-06 후속: 응답에는 구단명이 실리게 됐지만 이 판단은 그대로다. 응답의 구단명은 **선수마다의 소속 구단**이라 `Player.team`에서 나오지, 응원 구단 조회 결과에서 나오지 않는다. 다만 그 대가로 선수 목록 변환에서 `Player.team`이 LAZY 초기화된다(구단 수만큼 추가 SELECT) — fetch join 도입은 아직 안 했다.
