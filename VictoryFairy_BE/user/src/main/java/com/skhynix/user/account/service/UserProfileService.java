...
/**
 * 내 프로필 조회 서비스. 요구사항: {@code docs/requirements/user/me-profile.md}(USER-ME-7 ~ 22).
 */
@Service
...
public class UserProfileService {
     ...
     *
     * <p>DTO 조립을 이 트랜잭션 안에서 끝낸다(USER-ME-21) — 컨트롤러로 옮기면 {@code UserSupportTeam.team}
     ...
     *                      내부 PK). 탈퇴 계정은 그 해석 단계에서 걸러져 여기 닿지 않는다(USER-ME-10)
     */
    public UserAccountResponse getMyProfile(Long userAccountId) {
        ...
        // 응원 구단이 없으면 예외가 아니라 null 이다(USER-ME-16 안전망) — 활성 구단 행이 2개 이상인
        ...
        // 행이 없으면 0이다(USER-ME-19 안전망) — 배포~백필 사이 구간에서도 200을 유지하며, 행을
        // 만들지는 않는다(USER-ME-20).
