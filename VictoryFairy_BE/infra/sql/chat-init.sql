...
--     않는다(별도 관심사) — **같은 디렉터리의 `teams-init.sql`을 먼저 적용할 것.**
--     Step 3의 팀명 문자열은 py-collector 의 팀 차원 정의
--     (kbo_collector/dimensions.py 의 TEAMS)에서 확인한 실제 표기이며,
--     teams-init.sql 이 넣는 name 과 같은 출처라 서로 일치한다.
--     teams 가 비어 있거나 표기가 다르면 INSERT ... SELECT ... WHERE t.name = '…' 가
--     0행을 매칭해 조용히 아무것도 삽입하지 않는다(에러 없이 no-op이라 눈치채기
--     어려움 — 그래서 파일 맨 끝에 검증 쿼리를 둔다).
...
--     반드시 먼저 확인할 것.**
--     (2026-07-27 운영 DB 확인: uid VARCHAR(36) NOT NULL UNIQUE 존재 — 이 갭은 해소됨.)
-- ============================================================================
...
--
-- 참고(2026-07-27): 운영 DB에는 두 테이블이 이미 있다 — ddl-auto=update 부트스트랩
-- 사이클로 Hibernate가 만든 것이며, 실제 DDL을 떠서 대조한 결과 인덱스
-- (idx_chats_chatroom_created)와 FK 정책(team CASCADE / owner RESTRICT)까지 아래와
-- 동등했다. 따라서 현 운영 DB에서 Step 1은 no-op이다(새 환경 구축용으로 유지).
-- ============================================================================
