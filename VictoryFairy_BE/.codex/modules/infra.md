...
- ~~**`infra/sql/migrate-quiz-like-account-set-null.sql`**~~ — 해소(2026-08-18, devdb·운영 DB 양쪽 적용 완료). `quizzes_like`의 계정 FK를 `ON DELETE CASCADE`→`SET NULL`로 바꾼다(만료 데이터 정리 스케줄러가 계정을 하드 삭제할 때 추천 수가 조용히 깎이지 않게 하는 선행 조건). 상세는 `.codex/modules/domain.md`.
...
(`RedisQuizVoteTally`, dev도 실제 Redis에 쓴다. 상세는 `.codex/modules/quiz.md`).
...
- ~~**`game_statuses` 시드 0행**~~ — 해소(2026-08-05). `infra/sql/game-statuses-init.sql`을 `user` 앱이 dev·prod 모두
  기동 시 실행한다(`spring.sql.init`). 배포 전 수동 SQL 불필요. 상세는 `.codex/modules/domain.md`.
