...
3. **네이티브 공유** — 경기 결과·퀴즈 결과를 시스템 공유 시트로.
   인스타 스토리에 스티커를 얹어 보내는 경로는 이미 들어가 있다
   ([`instagram-share.md`](instagram-share.md)) — 웹뷰가 할 수 없는 일이라
   4.2 반론 재료로도 쓸 수 있다. 다만 Meta 앱 ID가 붙기 전에는 동작하지 않는다.
...
#### 식별자가 `com.victoryfairy.app`이 아니라 `com.victoryfairy.mobile`인 이유

원래 쓰려던 `com.victoryfairy.app`은 **양쪽 스토어 모두에 이미 선점돼 있다.** Apple
App ID는 개발자 계정을 통틀어 유일해야 하는데 다른 팀이 먼저 등록했고, Play도 앱 등록
단계에서 사용 중인 패키지명이라며 거부한다(공개된 앱은 없으니 비공개거나 과거에 삭제된
앱이 태운 이름이다). 어느 쪽도 회수 경로가 없다 — 도메인 소유권은 근거가 되지 않고,
Play는 앱이 삭제돼도 패키지명을 영구히 잠근다.

두 플랫폼이 같은 문자열을 쓸 필요는 없지만, 같으면 Firebase·딥링크 설정에서 관리할
값이 하나로 줄어서 맞췄다.

**패키지명은 Play에 한 번 올라가면 못 바꾼다.** 바꾸면 다른 앱이 되고 기존 설치본은
업데이트를 받지 못한다. 첫 업로드 전이 마지막 기회다.
