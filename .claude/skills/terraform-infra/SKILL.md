...
- **정제 파이프라인**: **서버리스**(2026-07-26 결정). 크롤(Lambda `kbo-collector`, EventBridge 상시) → **S3 이벤트** → 패턴 검열(Lambda) → **SQS**(`batch_size=5`) → LLM 검열(Lambda, Bedrock). 예산 상한은 **DynamoDB 원자적 카운터** + Bedrock Lambda **예약 동시성 1**. 멱등은 S3 `_manifest` 마커. 산출물은 **S3 에서 끝난다(MySQL 미사용)**. ⚠️ 폴링 컨트롤러·배치 Redis·Spot 회수 대응은 **전부 폐기됐다** — 트리거가 인프라 부품이 됐다.
...
- 노드그룹 IAM은 필수 관리형 정책만: `AmazonEKSWorkerNodePolicy`,
  ...
  `AmazonSSMManagedInstanceCore`(노드 SSH를 SSM 터널로만 열기 위함 — §6).
...
- **클러스터 내부 접근**: MySQL 보안그룹은 **EKS 노드 보안그룹에서 오는 3306만** 허용하고
  그 외 인입은 모두 차단합니다.
- **EKS 노드 SSH도 동일 패턴**: 노드그룹에 `remote_access`(키페어)를 붙이더라도 22를
  인터넷에 열지 말고 `source_security_group_ids`를 클러스터 SG로 한정하세요. 실제 접속은
  SG를 거치지 않는 SSM 포트포워딩(`scripts/db-tunnel.sh`)뿐입니다. ⚠ `remote_access` 추가는
  기존 노드그룹의 교체(재생성)를 유발합니다.
