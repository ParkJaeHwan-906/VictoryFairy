...
- `SecurityConfig` — stateless, `@Import(JwtVerificationConfig.class)`로 user의 JWT 검증 부품만 명시적으로 끌어옴(좁은 스캔 탓에 자동 감지 안 됨). `securityFilterChain`이 `JwtAuthenticationFilter`를 직접 `new`로 생성하며 `UserAccountRepository`(`:domain`, uid→id 조회)도 함께 넘김 — user의 필터 생성자가 바뀌면 여기도 같이 고쳐야 함. `/`, `/error`, `GET /health` 외 전부 인증 필수. 미인증 401도 user의 `RestAuthenticationEntryPoint`를 그대로 import해 등록(동일 커플링 — 생성자 바뀌면 같이 고칠 것)
...
- `SecurityConfig`가 user와 90% 동일해 중복이지만 **의도적으로 유지하기로 결정**: create엔 보안이 필요 없어 3번째 사례가 안 생기고, permitAll 규칙이 서로 달라 공통화하면 어색한 훅이 필요해진다. 다시 논의할 필요 없음
- `quiz`는 테스트 소스셋이 없어 401 응답 동작이 검증되지 않았다(user 쪽 테스트로만 커버). 열린 항목
