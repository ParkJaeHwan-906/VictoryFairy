...
데이터 EC2는 22 인바운드를 열지 않습니다. 다만 2026-07-27부터 3306/6379는
지정 CIDR(`/32`)에 한해 퍼블릭 EIP로 직결돼 있습니다 — **임시 조치이고 안정화 후
프라이빗 복귀 예정**입니다(경위와 되돌리기 비용은 [ARCHITECTURE.md](../docs/ARCHITECTURE.md) §3).
EKS 워커 노드도 22 인바운드를 인터넷에 열지 않습니다(소스 = 클러스터 SG 한정).
아래 터널은 그 예외와 무관하게 계속 유효한 경로입니다.
