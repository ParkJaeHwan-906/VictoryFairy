...
variable "bedrock_batch_post_size" {
  ...
  # 5 → 15 (2026-07-29). 예약 동시성이 1이라 처리량을 늘리는 합법적 레버는 이 값뿐이다.
  # 비용은 부수 효과로 함께 내려간다 — 비용이 사실상 전부 시스템 프롬프트(2,470토큰)라
  # 하루 소비액이 대략 $44.8/N 로 움직인다(N=5 → $8.97, N=15 → $3.0, 상한 $30).
  #
  ...
  #
  #   판정 1건 출력은 통과 약 25토큰, 폐기 40~50토큰이다. 4096 토큰의 수용량은
  #   대략 160단위(통과 위주) / 90단위(폐기 많을 때)다.
  #     N=15 평균(댓글 5개) → 90단위  ≈ 2,300토큰   여유 있음
  #     N=15 최악(댓글 20개) → 315단위 ≈ 7,900토큰  ← 초과 구간이 남아 있다
  #
  #   초과하면 응답이 잘려 "항목 수 불일치"가 되고, 2회 재시도 후 **배치 전건이
  #   폴백 통과 처리된다**(BRK-LLM-15). 실패로 떨어지는 게 아니라 미검열 콘텐츠가
  #   조용히 통과한다. 호출 비용은 재시도까지 3번 다 나간다.
  #
  #   ⚠ **최악 케이스는 이 두 값만으로는 못 막는다.** 게시글당 단위가 1~21 로 21배
  #     널뛰기 때문에 "게시글 수"로 배치를 자르는 방식 자체가 출력 크기를 보장하지
  #     못한다. 구조적 해법은 핸들러가 **판정 단위 수 기준으로 쪼개 호출**하는 것이며
  #     이는 앱(dev_ai) 변경이다. 그때까지는 CloudWatch 에서 "항목 수 불일치" 재시도
  #     로그를 감시할 것.
}
...
variable "bedrock_max_output_tokens" {
  ...
  # 2048 → 4096 (2026-07-29). 배치를 15로 올리면서 함께 올린다.
  # 출력 토큰은 **실제 생성분만 과금**되므로 상한 인상 자체의 비용은 사실상 없다.
  # 4096 이 이 모델의 천장이다 — 위 bedrock_max_output_tokens 변수의 validation 참고
