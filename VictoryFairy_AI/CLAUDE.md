...
2. **역할 분할** — 메인은 오케스트레이터만 하고, 실제 작업은 **역할별 서브에이전트 17개**(`.claude/agents/`)에 위임한다.
   ...
   - 공통: `context-keeper` · `cruft-sweeper`(구버전 흔적 **삭제** 전담 — 하네스의 유일한 뺄셈. `code-commenter` 와 정반대라 같은 파일에 동시에 돌리지 않는다) · `commit-writer`(사용자가 커밋을 요청했을 때만. push 안 함)
