...
카탈로그의 `difficulty` 값을 그대로 쓰고, 보상 두 축을 그 난이도로 결정한다 —
`pointReward`는 **`question-gen/config/scoring.yaml`의 `points` 표**를,
`bqReward`는 **같은 파일의 `bq` 표**를 그대로 옮겨 적는다(임의 값 부여 금지,
