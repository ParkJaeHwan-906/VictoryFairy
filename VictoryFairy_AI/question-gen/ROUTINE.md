...
| 공통 문항 (하루 1묶음) | 특정 팀에 치우치지 않는 것만 — 리그 전체 순위·역대 팀 기록·통산 기록·트렌딩. **경기가 없는 날은 범위를 넓힌다(아래 "경기가 없는 날" 절)** | `gameId: null`, `teamCodes: []` |
...
**`perGame`은 곧 구단별 문항 수다** — 아래 귀속 축이 양 팀을 모두 담으므로 한
경기 묶음은 양 팀이 나눠 갖는 것이 아니라 각 팀에 통째로 귀속된다. 각 구단이
매일 1경기씩 뛰므로 `perGame` 합계가 그날 그 구단 팬이 받는 경기 문항 수다.

**성공 기준은 사용자 1인이 받는 문항 수**이지 그날 발행 총량이 아니다 — 총량이
아무리 많아도 한 팬이 받는 건 자기 팀 경기 묶음 하나와 공통 묶음뿐이므로
1인 체감은 `perGame 합계 + common 합계`다. 목표치는 `scoring.yaml`의 조정 이력
주석에 근거와 함께 적혀 있다.

**경기가 없는 날은 공통 묶음이 그날의 전부다.** 월요일과 전 경기 취소일에는
경기 문항이 0이라 1인 체감이 `common` 합계로 떨어진다(그런 날의 실측은
`scoring.yaml` 조정 이력 표 참고 — 목표 미달이었다). 그런 날은 공통 묶음의
대상 엔티티 범위를 넓혀 `common` 슬롯을 끝까지 채운다:

- 평소 경기 묶음에서 쓰는 **PLAYER scope 템플릿**(`CAREER_PATH`·`MEME_ORIGIN`·
  `RECORD_OX`)을 공통으로 돌린다. 위키에 리그 전체 로스터 규모가 등재돼 있어
  재료는 충분하다.
- **TEAM scope 템플릿**(`STREAK_CURRENT`·`HOME_AWAY_SPLIT`·`RECENT_VS_EARLY`)도
  10개 구단에 고루 쓰면 특정 팀 편중 없이 공통 조건을 만족한다.
- 이때도 `gameId: null`·`teamCodes: []`는 그대로다 — 귀속 축을 비워야 전 구단
  팬에게 나간다. 한 팀에 쏠리지 않도록 구단을 고르게 분산한다.
- ⚠️ **중복 회피 창의 키가 달라진다.** 아래 "팀 단위로 적용"은 귀속 축
  (top-level `teamCodes`)을 전제로 하는데 공통 문항은 그 값이 비어 있다. 이런
  날은 `subject.teamCodes`·`subject.playerIds`(주제 축)를 키로 최근 7일과
  대조한다 — 그러지 않으면 어제 경기 문항으로 쓴 사실이 오늘 공통으로 재등장한다.
- ⚠️ 한 템플릿을 여러 엔티티로 반복하게 되므로 `verification-pass.md`의
  "같은 템플릿 3연속 폐기" 규칙에 걸리기 쉽다. 템플릿을 번갈아 배치해 연속을
  끊는다 — 물량을 채우려다 그 게이트에서 되레 잃는 것이 이 절의 실패 모드다.
