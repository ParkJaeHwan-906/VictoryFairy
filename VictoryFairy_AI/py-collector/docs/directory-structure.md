...
| 경로 | 역할 |
|---|---|
| **lambda/** | 서버리스 크롤(호출당 과금). `handler.py`(코어 호출 어댑터), `Dockerfile`+`requirements.txt`(lxml 네이티브 → 컨테이너 이미지), `README.md`. **이미지에 구워지는 것만** — ECR·Lambda·EventBridge·IAM 정의는 `VictoryFairy_Infra/collector-lambda/`(dev_infra) 소유. MySQL엔 안 씀 |
...
| Lambda 스케줄/환경변수 | `VictoryFairy_Infra/collector-lambda/` (dev_infra — 이 리포 아님) |
| Lambda 잡 분기(새 `job` 값) | `deploy/lambda/handler.py` + 위 스케줄 둘 다 |
