...
|---|---|---|
| `game_result` | `read_game_results` | **MySQL** (`games`·`teams`·`game_statuses`·`stadiums`·`game_lineups`) |
...
| # | 갭 | 근거 | 영향 |
|---|---|---|---|
| **G1** | ~~Lambda는 MySQL에 직접 적재하지 않는다~~ → **해소**: VPC 안 전용 함수 `kbo-collector-db`가 records/registrations를 운영 MySQL에 적재(`handler.py` DbSink 라우트 + `lambda_db.tf`). S3 잡 함수는 여전히 DB 미접근 | `handler.py`, `VictoryFairy_Infra/collector-lambda/lambda_db.tf`(dev_infra) | — |
| **G2** | ~~MySQL 적재(records/registrations) 스케줄러 없음~~ → **해소**: EventBridge 스케줄(records 03:30 KST·registrations 11:00 KST → `kbo-collector-db`). export 잡 스케줄은 여전히 없음 | `VictoryFairy_Infra/collector-lambda/lambda_db.tf`(dev_infra) | export 자동화만 잔여 |
...
| **G4** | ~~스키마 소유권 충돌~~ → **해소**: 운영 서비스 스키마(domain JPA, BIGINT PK)로 단일화. 수집기는 소스 자연키 컬럼(teams.code/players.kbo_player_id/games.naver_game_id)으로 upsert만. 구 스키마는 `migrate-legacy-collector.sql`로 제거 | domain JPA 엔티티(dev_be) | 선수 상세(등번호·생년월일 등)·박스스코어 스탯은 저장 안 함(운영 결정) |
