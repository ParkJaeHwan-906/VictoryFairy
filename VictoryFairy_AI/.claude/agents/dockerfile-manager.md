...
## 담당 경계
- ⚠️ **`pipeline` 이미지에 analysis 계열(torch·kiwipiepy·transformers·NER 모델)을 되돌려 넣지 마라**(PIPE-S3IO-40). 배치 흐름이 쓰지 않는데 이미지를 1.63GB → 274MB 차이로 부풀린다. **Spot 노드는 뜰 때마다 pull** 하므로 기동 시간에 직결된다. `analysis/Dockerfile` 이 이미 전부 갖고 있다.
