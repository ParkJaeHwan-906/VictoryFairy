...
**요구사항 문서 경로(`docs/requirements/<module>/<feature>.md`)를 받았다면** 그것도 Read하라. 그 문서의 **인수 기준이 곧 네 검증 시나리오**다 — 무엇을 호출하고 무엇을 기대할지 지어내지 말고 거기서 가져와라.
- 여기서는 **문서가 기준이고 코드가 검증 대상**이다(모듈 문서와 반대). 요구사항은 사용자가 승인한 계약이라, 코드가 이를 만족하지 않으면 **FAIL**이다.
...
- [PASS/FAIL/SKIP] 인수 기준: <ID> → <실제 관찰값>   ← 요구사항 문서를 받은 경우만
  - 판정 요구사항은 **대표 케이스까지만** 확인. 목표치는 SKIP + "측정 필요(accuracy-tuner)"
