...
argument-hint: "[validation|bedrock|analysis|pipeline|docker] (생략 시 현재 작업 모듈/diff로 추정)"
---
...
| `bedrock` | **module-verifier** — import → 기동 검증 → 판정 서비스 호출. ⚠️ **실호출은 과금된다** — 기본은 `BEDROCK_DRY_RUN=true`, 실호출이 필요하면 먼저 보고 |
...
- 불명확하면 `git diff --name-only`로 판단: `validation/**`·`bedrock/**`·`analysis/**`·`pipeline/**` → 코드, `*/Dockerfile`·`docker-compose.yml` → docker.

## 전달할 것
- **무엇을 바꿨는지**(파일/엔드포인트/기대값). 이게 없으면 검증자가 무엇을 확인해야 할지 모른다.
- 코드면 `module=<validation|bedrock|analysis|pipeline>`.
