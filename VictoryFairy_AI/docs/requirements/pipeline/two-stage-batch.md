...
| PIPE-2SB-14 | 유비쿼터스 | THE 시스템 SHALL LLM 판정 로직을 `bedrock` 모듈 서비스에 위임하고, **하나 이상의 게시글의 단위들을 묶은 배치**(본문·댓글 **혼합 가능**)를 전달하되 **항목마다 `unit_kind`(`"body"` \| `"comment"`)를 동반**한다 | 러너는 S3 I/O·단위 분해·배치 조립·결과 라우팅만. **"한 게시글"로 한정하면 PIPE-2SB-71(게시글 N개분)과 직접 충돌**하므로 열었다. 실측상 게시글당 단위가 **1.4개**라 게시글 1개씩 보내면 호출마다 캐시 read 약 1,500토큰이 붙어 **배칭 이점이 사실상 사라진다**(PIPE-2SB-64 가 지적한 바로 그 문제). 배치가 **이종 혼합**이므로 `unit_kind` 는 배치 인자가 아니라 **항목별 인자**여야 한다(BRK-LLM-1) |
| PIPE-2SB-71 | 유비쿼터스 | THE 시스템 SHALL 한 호출에 묶는 크기를 **설정값**으로 읽되, 그 단위를 **"게시글 N개분"** 으로 센다 | 자연스러운 배치 경계가 **게시글 하나**다(그 게시글의 본문+댓글이 함께 간다). "단위 N개"로 세면 게시글이 배치 경계에 걸쳐 잘리고, 그러면 **게시글 단위 원자성(PIPE-2SB-18)과 예산 확인 시점(PIPE-2SB-74)이 둘 다 깨진다.** 기본값은 문서에 박지 않는다 — 최적값은 판정 품질을 보고 `accuracy-tuner` 가 정한다. 비용 근거: 캐시 read 약 1,500토큰이 호출마다 붙으므로 묶을수록 단위당 부담이 준다 |
...
| PIPE-2SB-59 | 이벤트 | WHEN 어떤 단위가 폴백 통과(`fallback=true` — BRK-LLM-15 실제 장애 또는 BRK-LLM-16b DRY_RUN)로 처리되면, THE 시스템 SHALL 그 사실을 **사유 문구(`message`)와 함께** success 객체에 기록한다 | `fallback` 만 남기면 **Bedrock 장애와 DRY_RUN 이 산출물에서 구분되지 않는다**. 폴백 통과는 `is_valid=true` 라 기록이 없으면 **"이 데이터는 2차 검열을 안 거쳤다"를 사후에 알 수 없다** |
...
| PIPE-2SB-72b | 유비쿼터스 | THE 패턴 러너 SHALL 한 게시글에 대해 **`마커 기록 → SADD pending:bedrock → SREM pending:pattern`** 순서를 지킨다 | 다음 단계에 넣기 전에 현재 단계에서 빼면, 그 사이 크래시 시 **양쪽 Set 어디에도 없는 게시글**이 생긴다. 이 순서면 크래시 시 두 Set 에 동시에 존재할 뿐이고, 재처리 → 마커 skip → `SADD`(멱등) → `SREM` 으로 수렴한다 |
...
| PIPE-2SB-44 | 이벤트 | WHEN Bedrock 러너가 종료되면, THE 시스템 SHALL LLM 호출 횟수·본문/댓글 단위 수·폐기 단위 수·폴백 통과 건수·**누적 소비액과 캐시 적중 토큰 수**·**예산으로 시작하지 못한 게시글 수**를 요약 로그로 남긴다 | 캐시 적중 토큰이 0이면 PIPE-2SB-64/65 가 깨진 것이다. 미시작 건수는 PIPE-2SB-74 의 관측 지점 |
...
| PIPE-2SB-64 | 유비쿼터스 | THE 시스템 SHALL 판정 기준·케이스가 담긴 **시스템 프롬프트 접두부에 캐시 breakpoint** 를 둔다 | 연속 호출에서 `cache_read_input_tokens > 0`. **최적화가 아니라 계약이다** — 판정 텍스트는 20~50토큰인데 시스템 프롬프트가 약 1,500토큰이라 **호출 비용의 대부분이 접두부**다 |
| PIPE-2SB-65 | 유비쿼터스 | THE 시스템 SHALL **판정 대상 텍스트를 캐시 breakpoint 뒤에** 배치한다 | 캐시는 **프리픽스 매칭**이다. 앞에 오면 프리픽스가 매 호출 달라져 **캐시가 전혀 동작하지 않는다** |
...
- **모델 단가가 미확인 가정치다.** Bedrock 페이지에서 확인된 건 Sonnet 5(프로모 $2/$10)뿐이고, 쓰기로 한
  **Sonnet 4 단가는 Anthropic 1P 단가를 파트너 단가로 가정**한 값이다(AWS Pricing API 는 조직 SCP 로 차단).
  `bedrock_spend_usd` 는 **추정 소비액이지 청구액이 아니다** — **첫 수일간 AWS Cost Explorer 실청구액과 대조**하고
  어긋나면 단가 설정값(PIPE-2SB-62)을 갱신한다. 단가를 코드 상수로 두지 않은 이유가 이것이다.
...
6. **모델** = 본문·댓글 **단일** `apac.*` Claude **Sonnet 4**. → BRK-LLM-5/6.
7. **데이터 범위** = **APAC 이내 유지**(`global.*` 금지). → BRK-LLM-6b/6c.
