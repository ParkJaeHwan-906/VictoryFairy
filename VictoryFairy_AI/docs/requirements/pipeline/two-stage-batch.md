...
>
> ## ⚠️ v2 개정 — 실행 환경이 EKS 배치에서 Lambda 서버리스로 바뀌었다
>
> **계기**: 크롤러가 이미 Lambda(`kbo-collector`, EventBridge `rate(10 minutes)`)로 돌고
> 있었는데 v1 은 "02:00 에 크롤 Job 을 띄운다"를 전제했다. **Lambda 는 VPC 밖이라
> `batch-redis`(ClusterIP)에 도달할 수 없어** 크롤러가 `SADD pending:pattern` 을 하지
> 못하고, 그러면 `SCARD >= 1000` 트리거가 **영원히 발동하지 않는다.** "크롤 Job 종료"라는
> 사건도 없어 잔여분 flush 도 걸리지 않는다.
>
> **결정**: 크롤러를 EKS 로 옮기는 안 대신 **정제를 Lambda 로 통일**했다. 단계 전이를
> S3 이벤트 알림과 SQS 이벤트 소스 매핑이 담당하므로 **우리가 짜야 할 폴링 컨트롤러
> 12개 조항이 통째로 사라진다.** 근거는 `VictoryFairy_Infra/docs/ARCHITECTURE.md` §4.
>
> **영향 범위**: 86조항 중 **23건(27%)**. 컨트롤러 12 · 작업 집합 6 · 비용 카운터 2 ·
> Spot 전제 3. **나머지 73%(판정 규칙·S3 키 규약·마커 멱등·예산 금액 계약)는 실행 환경과
> 무관해 그대로 살아남는다.**
>
> ⚠️ **폐기된 조항의 번호는 재사용하지 않는다.** 테스트·커밋·리뷰가 그 ID 를 참조하고
> 있어서, 당기는 순간 남의 문서가 다른 요구사항을 가리킨다.
...
| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| PIPE-2SB-74 | 이벤트 **(v2 개정됨)** | WHEN 어떤 배치의 호출을 시작하기 전에, THE Bedrock Lambda SHALL 누적 소비액을 확인해 상한 이상이면 **그 배치를 처리하지 않고 전건을 큐에 남긴다** | ⚠️ v1 은 "남은 게시글도 시작하지 않는다" 였다(배치 러너가 루프를 돌았으므로). Lambda 는 배치 단위로 뜨므로 **그 호출의 메시지 전건을 `batchItemFailures` 로 돌려준다** — 삼키면 그 게시글들은 영구 미판정이 된다. 카운터가 날짜별이라 다음 날 자연히 재개된다. ⚠️ **카운터 접근 실패는 "상한 도달"과 다르다** — 그때는 예외를 올려 함수를 실패시킨다(0으로 눙기면 상한이 조용히 사라진다) |
...
| PIPE-2SB-20 | 예외 **(v2 개정됨)** | IF 함수 실행이 타임아웃·오류로 중단되면, THEN THE 시스템 SHALL 진행 중인 게시글의 마커를 쓰지 않고 종료한다 | ⚠️ v1 은 SIGTERM(Spot 회수) 전제였다. Lambda 에는 Spot 회수가 없고 대신 **타임아웃·메모리 초과·미처리 예외**가 같은 자리를 차지한다. 마커가 없으면 재시도·재배달이 그 게시글을 다시 처리한다 — 계약의 실질은 그대로다 |
...
| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| PIPE-2SB-26 | 유비쿼터스 **(v2 개정됨)** | THE 시스템 SHALL 배치 상태를 **DynamoDB 테이블 1개**(파티션 키 `batch_date`, 속성 `spend_usd`)로만 둔다 | ⚠️ v1 은 `batch-redis` 의 Set 2개(`pending:*`) + 실수 키 1개였다. **Set 2개는 폐기됐고**(단계 전이를 S3 이벤트·SQS 가 담당) 누적 소비액만 남았다. 저장소가 Redis 에서 DynamoDB 로 바뀐 이유는 **Lambda 가 VPC 밖이라 ClusterIP Redis 에 도달하지 못하기 때문**이다. 파티션 키를 날짜로 두면 **일 단위 리셋이 구조적으로 성립한다** — 날짜가 바뀌면 다른 아이템이라 초기화 작업이 없다(구 `emptyDir` Redis 가 주던 성질과 같다). TTL 30일: 스토리지가 무료(첫 25GB)라 지워서 아낄 것이 없고 **추정치와 실청구액을 대조할 이력**이 필요하다. ⚠️ TTL 을 처리 창보다 짧게 잡으면 **하루 중간에 상한이 리셋된다** |
| PIPE-2SB-72 | 유비쿼터스 **(v2 개정됨)** | THE 시스템 SHALL 게시글 처리 완료를 **`산출물 기록 → 마커 기록`** 순서로 반영한다 | ⚠️ v1 은 `S3 마커 기록 → Redis SREM` 이었다. SREM 이 사라지면서 **살아남은 순서 계약이 이것**이다 — 마커가 먼저 찍히면 산출물이 없는데 완결로 보여 **재처리가 영영 막힌다** |
| PIPE-2SB-72b | 유비쿼터스 **(v2 개정됨)** | THE 패턴 Lambda SHALL 한 게시글에 대해 **`산출물·마커 기록 → (success 면) SQS 전송`** 순서를 지킨다 | ⚠️ v1 의 `SADD`/`SREM` 순서 계약을 대체한다. 큐 전송이 마커보다 **먼저** 가면 그 사이 실패 시 큐에는 있는데 마커가 없어, 재시도가 같은 게시글을 또 넣어 **중복 판정·중복 과금**이 된다. 반대 순서의 대가는 큐 전송 실패 시 2단계 누락인데, **누락은 백필로 복구되지만 중복 과금은 되돌릴 수 없다** — 그래서 이 순서다 |
| PIPE-2SB-73 | 유비쿼터스 **(v2 개정됨)** | THE 시스템 SHALL **S3 마커를 진실의 원천**으로 취급하고, 큐·카운터를 그에 종속된 수단으로 본다 | v1 의 "Redis 작업 집합은 최적화 수단" 을 승계한다. S3 이벤트·SQS 재배달이 모두 **최소 한 번(at-least-once)** 이므로 같은 게시글이 두 번 이상 도착할 수 있다 — 마커가 그것을 흡수한다. ⚠️ **비용 카운터는 예외다**(PIPE-2SB-74) — 대체 정본이 없어 접근 실패 시 중단한다 |
| PIPE-2SB-25 | **(v2 폐기됨)** | ~~이벤트: WHEN 크롤 워커가 게시글 1건을 S3 에 기록하면, THE 크롤 워커 SHALL 그 게시글 키를 `pending:pattern` 에 `SADD` 한다 | 기록 성공 후 추가(유령 항목 금지). 같은 키를 두 번 넣어도 Set 이라 중복되지 않는다~~ → **대체**: 크롤러는 S3 에 쓰기만 한다. 그 객체 생성이 곧 다음 단계의 트리거다(PIPE-2SB-79) — 크롤러가 정제 단계를 알 필요가 없어졌다 |
| PIPE-2SB-76 | **(v2 폐기됨)** | ~~예외: IF 크롤 워커의 `SADD` 가 실패하면, THEN THE 크롤 워커 SHALL 최대 3회(가정) 재시도하고, 끝내 실패하면 에러 로그를 남기되 크롤은 계속한다 | S3 기록은 이미 끝났으므로 데이터 유실은 아니다. 다만 그 게시글은 `pending:pattern` 에 없어 트리거에 반영되지 않고(PIPE-2SB-28·32 의 조건에도 안 걸려) 그 배치에서 정제되지 않는다 — PIPE-2SB-73 이 다루는 것은 러너의 Set 실패뿐이라 이 경로가 비어 있었다. 크롤을 중단시키지 않는 이유: 입력 수집이 정제 트리거보다 우선한다~~ → **대체**: `SADD` 자체가 없다. 큐 전송 실패는 예외로 올려 Lambda 재시도에 맡긴다(PIPE-2SB-80) |
| PIPE-2SB-27 | **(v2 폐기됨)** | ~~상태: WHILE 배치가 진행되는 동안, THE 컨트롤러 SHALL `SCARD pending:pattern`·`SCARD pending:bedrock`·`bedrock_spend_usd` 를 60초(가정) 주기로 폴링한다 | 폴링 주기가 곧 트리거 지연 상한. 비용 키를 안 보면 PIPE-2SB-68 게이트가 작동하지 않는다~~ → **대체**: S3 이벤트 알림과 SQS 이벤트 소스 매핑이 단계 전이를 담당한다 — 폴링할 대상이 없다 |
| PIPE-2SB-28 | **(v2 폐기됨)** | ~~이벤트: WHEN `SCARD pending:pattern` 이 1000 이상으로 관측되면, THE 컨트롤러 SHALL 패턴 단계 Job 을 1개 생성한다 | `victoryfairy-refine-pattern` Job 생성. 차감 규칙은 없다 — 처리하면 Set 에서 빠지므로(PIPE-2SB-72) 개수가 저절로 줄어든다~~ → **대체**: S3 `ObjectCreated` 이벤트가 게시글 1건당 패턴 Lambda 를 호출한다(PIPE-2SB-79) |
...
| PIPE-2SB-30 | **(v2 폐기됨)** | ~~상태: WHILE 같은 단계의 Job 이 실행 중인 동안, THE 컨트롤러 SHALL 그 단계의 새 Job 을 생성하지 않는다 | 단계별 동시 실행 최대 1. 작업 집합은 락이 아니다(항목을 원자적으로 점유하는 계약이 없다) — 두 러너가 뜨면 같은 게시글을 중복 처리해 Bedrock 단계에서 중복 과금이 난다~~ → **대체**: Lambda 동시성은 AWS 가 관리한다. Bedrock 함수는 예약 동시성 1로 직렬화한다(PIPE-2SB-83) |
...
| PIPE-2SB-32 | **(v2 폐기됨)** | ~~이벤트: WHEN 크롤 Job 이 종료되고 `pending:pattern` 이 비어 있지 않으면, THE 컨트롤러 SHALL 1000 미만이어도 마지막 패턴 Job 을 생성한다(잔여분 flush) | 크롤 종료 시 잔여 340건 → 패턴 Job 1개 생성, 340건 처리. 카운터 조작은 하지 않는다(구 `SET 0` 폐기) — 처리되면 Set 에서 빠진다~~ → **대체**: "크롤 종료"라는 사건이 없다 — 크롤러가 상시 실행이고 각 객체가 독립적으로 흐른다 |
| PIPE-2SB-33 | (폐기됨) | ~~패턴 Job 완료마다 Bedrock Job 생성~~ | 번호 재사용 금지. 대체: PIPE-2SB-49 |
| PIPE-2SB-34 | **(v2 폐기됨)** | ~~예외: IF 컨트롤러가 Redis 또는 k8s API 에 접근하지 못하면, THEN THE 컨트롤러 SHALL 해당 폴링 주기를 건너뛰고 에러 로그를 남긴 뒤 다음 주기에 재시도한다 | 일시 장애로 컨트롤러가 죽지 않는다~~ → **대체**: Lambda 재시도·SQS 재배달이 대신한다 |
| PIPE-2SB-47 | **(v2 폐기됨)** | ~~이벤트: WHEN 패턴 러너가 어떤 게시글의 마커를 기록했고 그 게시글의 success 객체가 생성됐으면, THE 패턴 러너 SHALL 그 키를 `pending:bedrock` 에 `SADD` 한다 | 마커 기록 성공 후 추가. 전건 폐기되어 success 미생성(PIPE-S3IO-19)인 게시글은 넣지 않는다 — Bedrock 이 볼 입력이 없다~~ → **대체**: 패턴 Lambda 가 통과분을 **SQS 로 보낸다**(PIPE-2SB-80). 큐가 대기 목록 역할을 한다 |
...
| PIPE-2SB-49 | **(v2 폐기됨)** | ~~이벤트: WHEN `SCARD pending:bedrock` 이 1000 이상으로 관측되면, THE 컨트롤러 SHALL Bedrock 단계 Job 을 1개 생성한다 | Bedrock 은 패턴 Job 완료 여부가 아니라 자기 작업 집합으로 뜬다. 차감 없음(구 `DECRBY 1000` 폐기)~~ → **대체**: SQS 이벤트 소스 매핑이 `batch_size` 만큼 모아 Bedrock Lambda 를 호출한다(PIPE-2SB-81) |
| PIPE-2SB-50 | **(v2 폐기됨)** | ~~이벤트: WHEN 크롤 Job 이 종료되고 `pending:pattern` 이 비었으며 패턴 Job 이 `Complete` 이고 `pending:bedrock` 이 비어 있지 않으면, THE 컨트롤러 SHALL 1000 미만이어도 마지막 Bedrock Job 을 생성한다(잔여분 flush) | 패턴 잔여 flush(PIPE-2SB-32) 다음에 Bedrock 잔여 flush 가 이어져야 배치가 소진된다. 카운터 조작 없음. ⚠️ 예산이 소진된 상태에서는 PIPE-2SB-68 이 우선한다 — flush Job 도 만들지 않는다~~ → **대체**: 위와 같음 — 잔여분이라는 개념이 없다. 큐에 남으면 그대로 배달된다 |
| PIPE-2SB-68 | **(v2 폐기됨)** | ~~상태: WHILE `bedrock_spend_usd` 가 상한(PIPE-2SB-60) 이상인 동안, THE 컨트롤러 SHALL Bedrock Job 을 생성하지 않는다 — 게이트 Job(PIPE-2SB-49)이든 잔여분 flush(PIPE-2SB-50)든 예외 없다 | 이 금지는 PIPE-2SB-49·50 의 생성 의무보다 우선한다. 실질적인 제동이 여기다 — 러너만 멈추면(PIPE-2SB-63) 새 Job 이 계속 떠서 게시글은 하나도 처리하지 못한 채 공회전한다~~ → **대체**: 예산 게이트는 Bedrock Lambda 자신이 호출 전에 확인한다(PIPE-2SB-74) |
| PIPE-2SB-35 | **(v2 폐기됨)** | ~~이벤트: WHEN 06:00 KST 에 도달하고 크롤 Job 이 아직 실행 중이면, THE 컨트롤러 SHALL 크롤 Job 을 강제 종료한다 | 06:00 은 크롤 종료 시각이지 배치 종료 시각이 아니다(사용자 확정)~~ → **대체**: 크롤 Job 이 없다 — 크롤러는 Lambda 로 상시 실행된다 |
...
| PIPE-2SB-35c | **(v2 폐기됨)** | ~~이벤트: WHEN `pending:pattern` 이 비고 크롤·패턴 Job 이 모두 종료했으며, `pending:bedrock` 이 비었거나 `bedrock_spend_usd` 가 상한 이상이면, THE 컨트롤러 SHALL 실행 중인 Bedrock Job 의 종료를 기다린 뒤 배치 종료로 판정하고 미처리 건수를 로그로 남긴 뒤 자신을 종료한다 | 예산 소진은 Bedrock 단계만 멈추는 조건이지 배치 전체의 수명이 아니다 — 그래서 예산 항목은 `pending:bedrock` 자리에만 걸린다. 예산 항목이 없으면 PIPE-2SB-68 게이트 때문에 그 Set 이 영영 안 비어 컨트롤러가 죽지 않는다. 키 초기화는 하지 않는다 — Redis 가 배치와 함께 사라지므로(PIPE-2SB-26) 초기화할 대상이 없다~~ → **대체**: "파이프라인 종료"라는 순간이 없다. 큐가 비면 Lambda 가 안 뜰 뿐이고 내릴 노드도 없다 |
| PIPE-2SB-36b | **(v2 폐기됨)** | ~~(폐기됨): ~~배치 시작 시 컨트롤러가 Redis 키를 초기화~~ — `batch-redis` 가 `emptyDir` 로 배치마다 새로 뜨므로 초기화 대상이 없다. "크롤 워커가 먼저 떠서 올린 값을 컨트롤러가 지운다"는 경합도 구조적으로 소멸한다 | 번호 재사용 금지. 대체: PIPE-2SB-26~~ → **대체**: 이미 v1 에서 폐기된 조항이다(중복) |

### E. 스케줄 · 배치 날짜

| ID | 유형 | 요구사항 | 인수 기준 |
|---|---|---|---|
| PIPE-2SB-36 | **(v2 폐기됨)** | ~~이벤트: WHEN 매일 02:00 KST 가 되면, THE 시스템 SHALL 야간 배치(크롤 Job + 컨트롤러 + `batch-redis`)를 시작한다 | CronJob `timeZone: Asia/Seoul` + `schedule: "0 2 * * *"`. 기존 03:15 스케줄은 제거~~ → **대체**: 02:00 기동이 없다 — 이벤트 구동이라 하루 종일 흐른다 |
...
| PIPE-2SB-42 | 이벤트 **(v2 개정됨)** | WHEN 함수가 콜드 스타트로 뜨면, THE 시스템 SHALL 이미지 로딩을 포함해 **10초(가정)** 이내에 첫 게시글 처리를 시작한다 | ⚠️ v1 은 "Spot 노드 0→1 증설 + 이미지 pull 120초" 였다. Lambda 는 노드 증설이 없고 **이미지를 캐시하며 청크 단위로 지연 로딩**하므로 자릿수가 다르다. 이미지는 약 1.09GB 지만 그중 우리 코드·의존성은 39MB 이고 나머지는 베이스 이미지다 |
...
| PIPE-2SB-61 | 유비쿼터스 **(v2 개정됨)** | THE 시스템 SHALL 누적 소비액을 DynamoDB 의 `spend_usd` 에 두고, 각 호출 후 응답 `usage` 토큰 수 × 단가로 계산한 금액을 **`UpdateItem ... ADD`** 로 누적한다 | `ADD` 는 `INCRBYFLOAT` 와 같은 원자적 증분을 준다. ⚠️ **원자성만으로는 상한을 못 지킨다** — 동시에 뜬 함수들이 각자 "아직 여유 있음"을 읽고 함께 넘길 수 있다. 그래서 Bedrock Lambda 의 예약 동시성을 1로 묶어 직렬화한다(PIPE-2SB-83). 누적값은 **추정 소비액이지 청구액이 아니다**(토큰 집계 × 단가) — 첫 수일간 Cost Explorer 와 대조할 것. 단가는 약관 요율표로 확인됨(입력 $3 / 출력 $15 per 1M) |
...
| PIPE-2SB-79 | 이벤트 **(v2 신규)** | WHEN 크롤러가 `community/{source}/{date}/{postId}.json` 를 S3 에 기록하면, THE 시스템 SHALL **S3 `ObjectCreated` 이벤트로 패턴 Lambda 를 게시글 1건 단위로 호출**한다 | v1 의 "`SCARD pending:pattern` >= 1000 게이트"를 대체한다. **크롤러는 정제 단계를 알 필요가 없다** — S3 에 쓰기만 하면 된다. ⚠️ 이벤트의 객체 키는 **URL 인코딩**돼 있다(공백이 `+`, 한글이 `%XX`) — 디코드하지 않으면 GetObject 가 `NoSuchKey` 로 실패한다. ⚠️ S3 이벤트는 **최소 한 번** 배달이라 같은 객체로 두 번 이상 호출될 수 있다(PIPE-2SB-82) |
| PIPE-2SB-80 | 이벤트 **(v2 신규)** | WHEN 패턴 Lambda 가 success 객체를 만들었으면, THE 패턴 Lambda SHALL 그 게시글 키를 **SQS 큐로 전송**한다 | v1 의 `SADD pending:bedrock`(PIPE-2SB-47)을 대체한다. 전건 폐기된 게시글은 보내지 않는다 — Bedrock 이 볼 입력이 없다. 순서는 PIPE-2SB-72b(마커 뒤). 큐 URL 이 없으면 **예외로 올린다** — 조용히 넘어가면 패턴 통과분이 2단계로 영영 넘어가지 않는다 |
| PIPE-2SB-81 | 유비쿼터스 **(v2 신규)** | THE 시스템 SHALL SQS 이벤트 소스 매핑의 `batch_size` 를 **한 번의 모델 호출에 묶는 게시글 수**로 삼고, **2 이상**으로 둔다 | v1 의 "게시글 N개분"(PIPE-2SB-71)을 인프라 부품으로 옮긴 것이다. ⚠️ **1이면 상한을 넘긴다** — 시스템 프롬프트 2,470토큰이 호출마다 붙어 하루 $44.8 로 $30 을 초과한다. 5건이면 $8.97 이다. 값은 Terraform `bedrock_batch_post_size`(기본 5)이며 `validation` 블록이 하한을 강제한다 |
| PIPE-2SB-82 | 유비쿼터스 **(v2 신규)** | THE 시스템 SHALL 두 Lambda 모두 처리 전에 **완결 마커를 확인해 이미 있으면 건너뛴다** | S3 이벤트·SQS 재배달·Lambda 재시도가 모두 최소 한 번 배달이라 **중복 호출이 정상 경로**다. Bedrock 쪽에서는 이 확인이 곧 **중복 과금 방지**다 — 마커 없이 재판정하면 결과는 같아도 돈은 두 번 나간다 |
| PIPE-2SB-83 | 유비쿼터스 **(v2 신규)** | THE 시스템 SHALL Bedrock Lambda 의 **예약 동시성을 1로 고정**한다 | 예산 상한의 **두 번째 겹**이다. DynamoDB 원자적 증분(PIPE-2SB-61)만으로는 동시에 뜬 함수들이 각자 "아직 여유 있음"을 읽고 함께 넘긴다. ⚠️ **처리량이 부족하면 `batch_size` 를 키우지 동시성을 올리지 마라** — 올리는 순간 상한이 무의미해진다 |
| PIPE-2SB-84 | 이벤트 **(v2 신규)** | WHEN SQS 배치 안에서 일부 메시지만 실패하면, THE Bedrock Lambda SHALL **실패한 메시지 ID 만 반환**한다(`ReportBatchItemFailures`) | 성공분까지 재배달되면 **같은 게시글에 모델 호출이 두 번 나간다** — 마커가 결과는 막지만 돈은 이미 나간 뒤다. 인프라에서 `function_response_types` 로 켠다 |
| PIPE-2SB-85 | 예외 **(v2 신규)** | IF SQS 메시지가 깨졌거나 참조하는 패턴 산출물이 없으면, THEN THE Bedrock Lambda SHALL 그 메시지를 **성공 처리해 큐에서 제거**한다 | 재시도해도 낫지 않는 입력이다. 실패로 돌리면 `maxReceiveCount` 만큼 재배달되다 DLQ 로 가는데, 그동안 **정상 메시지의 처리를 지연시킨다**. 패턴 산출물 부재는 정책 재적용으로 지워진 정상 경로이기도 하다 |
