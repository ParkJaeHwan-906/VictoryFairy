...
| PIPE-S3IO-40 | 유비쿼터스 **(신규 2026-07-26)** | THE 시스템 SHALL 배치 이미지(`pipeline/Dockerfile`)에 **현행 흐름이 실제로 쓰는 모듈만** 담고, 배선에서 빠진 analysis 계열(`analysis/` · `kiwipiepy` · `transformers` · `torch` · NER 모델)은 **포함하지 않는다** | 배치 흐름은 `run_validation` → `run_bedrock` 2단계뿐인데(PIPE-S3IO-28) 이미지가 torch(CPU 빌드)와 KoELECTRA 모델까지 담아 **1.63GB** 였다 — **전부 쓰지 않는 무게다.** ⚠️ **근거가 v2 에서 약해졌다** — 당시 이유는 "Spot 노드가 뜰 때마다 pull 한다" 였는데, Lambda 전환으로 노드가 없어지고 Lambda 는 이미지를 캐시·지연 로딩한다. 그래도 조항은 유지한다: **쓰지 않는 의존성을 담을 이유가 없고**, torch·NER 모델은 콜드 스타트와 빌드 시간을 늘린다. `analysis/Dockerfile` 이 이미 torch·모델·코드를 모두 갖고 있어 배치 이미지의 사본은 **순수 중복**이었다. ⚠️ **결과: `run_analysis`·`run_aggregate` 는 이 이미지로 돌지 않는다** — `analysis/Dockerfile` 이미지나 로컬 `.venv` 를 쓴다. PIPE-S3IO-29(코드 보존)와 충돌하지 않는다: 파일은 저장소에 그대로 있다. analysis 가 배선에 복귀하면 이 조항을 재검토한다. **v2 추가**: 이미지 베이스가 `public.ecr.aws/lambda/python` 로 바뀌었다 — 일반 python 이미지는 Runtime Interface Client 가 없어 Lambda 로 뜨지 않는다. 아키텍처는 `linux/arm64` 로 고정하며 **Terraform 의 `architectures` 와 반드시 일치**해야 한다(다르면 함수 생성 자체가 실패) |
