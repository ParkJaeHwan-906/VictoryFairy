...
## 11. 확장: 경기 팩·예측 퀴즈 (2026-08-04 추가)

목표: "시청 중 재미". 유저는 응원팀 경기를 보는 동안 그 경기와 관련된 문제를
풀어야 하므로, 생성 단위를 하루 풀(pool)에서 **경기 팩**으로 확장한다.

### 11.1 경기 팩

- 아침 루틴이 오늘 스케줄의 각 경기에 대해 팩을 생성: 양팀 소속 엔티티만
  바인딩(선수 밈·기록·상대전적·순위), 편중 회피는 팀 내부에서만 적용.
- 물량: 팩당 6문항(초기값, 품질 보며 8~10으로 증량) × 일 5경기 + 공통 팩
  10문항 = 일 40문항 수준. LLM 한계비용 0(구독)이므로 비용 제약 없음. 세션
  소요 상한은 30분 → 60분으로 완화하고, 초과 시 팩 단위로 fail-closed.
- **태깅 계약**: 팩 문항은 `gameId`(기존 필드)와 `teamCodes: ["SS","LT"]`
