...
- **역할**: 유니코드 호환 문자 접기 · 소문자화 · 다중/단일 문자 치환 · 공백/특수문자 제거로 회피 표기를 표면형으로 복원(`preprocess`).
- **유니코드 호환 문자 접기**(`fold_compat`, 0단계): 전각 `ｓｉｂａｌ` · 수학 볼드 `𝘀𝗶𝗯𝗮𝗹` · 프락투르 `𝖘𝖘𝖎𝖇𝖆𝖑` · 원문자 `ⓢⓘⓑⓐⓛ` 처럼 **겉보기는 같고 코드포인트만 다른 표기**를 NFKC 로 접는다. 수기 나열은 새 변형마다 뚫리므로(프락투르가 실제로 통과했다) 표준 매핑에 맡긴다.
  - ⚠️ **한글 두 가지를 반드시 보호해야 한다.** NFKC 는 호환 자모 `ㅅ`(U+3145)을 조합용 자모 `ᄉ`(U+1109)로 바꾸는데, U+1109 는 특수문자 제거 정규식의 `ㄱ-ㅎ` 범위 밖이라 삭제되고 **초성 욕설이 전부 무력화**된다(실측: 검거 35,958건 → 17건). 그래서 호환 자모는 접기 대상에서 제외한다. 또 악센트 제거용 NFD 는 한글 음절까지 분해하므로(`시`→`ㅅㅣ`) **NFC 로 재조합**해 되돌린다.
  - 숫자·기호 leet(`s1b4l`·`씨@발`)은 유니코드 호환 문자가 아니라 NFKC 가 건드리지 않는다. `normalization.json` 의 치환 맵(13개)이 계속 담당한다.
