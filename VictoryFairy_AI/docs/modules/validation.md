...
- **다중 뷰 매칭**(`build_match_views`): 단일 정규화만으로는 한글 사이에 낀 기호·숫자(`씨@발` → `씨a발`)를 놓치므로, 같은 문장을 **6개 뷰로 변환해 각각 매칭**한다.
  | 뷰 | 변환 | 잡는 케이스 |
  |---|---|---|
...
- **흐름**: `build_match_views`로 6개 뷰 생성 → 각 뷰마다 (예외 표현 제거 → 패턴 검색) → 첫 매칭 시 폐기. **키보드 뷰는 `KEYBOARD_PATTERNS`(완성 음절만)**, 나머지는 `CATEGORY_PATTERNS` 사용. 어느 뷰에서도 안 걸리면 통과.
