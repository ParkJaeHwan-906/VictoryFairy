...
### 2. 비속어 패턴·사전
- **파일**: `core/patterns.py`, 로더 `core/resources.py`, 데이터 `core/data/banned_words.json` · `core/data/exceptions.json`(오탐 방지 whitelist)
- **역할**: 사전을 정규화 후 정규식으로 컴파일(길이 내림차순). 사전 파일만 고치면 반영.
- **카테고리**: `general`(욕설) · `sexual`(성적 표현) · `parent`(패드립) — 초성 욕설 포함.
  여기에 **비하 표현은 단어마다 독립 카테고리**를 둔다: `pyegeup`(폐급) · `disabled`(장애인) ·
  `daegari`(대가리) · `mental`(정신병자) · `gaessip`(개씹) · `aemchang`(앰창·엠창).
  어휘별 오탐 성향이 크게 달라(폐급 검거 15/오탐 36 vs 정신병자 7/5) 한 덩어리로 묶으면
  하나 때문에 전부 들어내야 한다. 라벨은 `services/validation.py` 의 `_CATEGORY_LABELS`.
...
## 한계 · 향후 과제
- **혼용 표기 오탐**: 한글/영어 뷰는 단어 경계를 없애므로, 한글 사이에 라틴/숫자를 억지로 끼운 비정상 표기(`역시b발라드`→`시발`)에서 드물게 오탐. 정상 표현은 `exceptions.json`으로 보정.
- **공백 제거로 생기는 충돌**: 모든 뷰가 공백을 지우므로 `유일한 남자`→`유일한남자` 가 `한남` 과 겹친다. **뷰만으로는 구분 불가**이고 `exceptions.json` 이 유일한 방어다. 다행히 겹침은 열려 보여도 대개 닫힌 집합(`한`+`남{자,성,편,매,녀,아}`)이라 예외로 닫힌다. 반대로 **닫히지 않는 어휘는 사전에 넣지 않는다** — `쳐먹`은 `이닝 좀 처먹어라` 처럼 사이에 말이 끼면 예외가 뚫려 제외했다.
- **예외 사전은 '원문' 뷰의 숫자를 못 막는다**: 예외어도 `preprocess()` 로 정규화해 컴파일하므로(`4`→`a`) 숫자를 보존하는 원문 뷰와 만나지 못한다. `4강 간거임` 이 `강간` 으로 걸리는 것이 이 경우(실측 4건/63,859건).
- **오탐 실측 기준**(2026-08-04): Bedrock 2차 검열이 전 유닛을 통과시킨 게시글 **63,859 유닛**을 정상 코퍼스로 삼아 측정한다. 현행 사전 기준 신규 오탐 **489건(0.766%)**. 과거의 "30문장 측정" 기준은 표본이 작아 폐기.
...
- 비하 표현은 기존 카테고리에 섞지 말고 **새 키를 만들고** `_CATEGORY_LABELS` 에 라벨을 추가한다. 되돌릴 때 그 단어만 빠진다.
- **넣기 전에 두 코퍼스로 재보고 넣는다**: S3 `validation/bedrock/failed`(abuse 판정 = 잡아야 할 것)와 `validation/bedrock/success` 마커가 붙은 게시글의 원본 `community/` 텍스트(= 걸리면 안 되는 것). 검거만 세고 오탐을 안 세면 통과율만 깎인다.
